...
public abstract class AIPlayer implements Player, Serializable {
    ...
    protected Direction calculateBestDirection(Snake snake, Point apple, int boardWidth, int boardHeight) {
        ...
        if (Math.abs(dx) > Math.abs(dy)) {
            if (dx > 0) priorityDirs.add(Direction.RIGHT);
            else priorityDirs.add(Direction.LEFT);
            if (dy > 0) priorityDirs.add(Direction.DOWN);
            else if (dy < 0) priorityDirs.add(Direction.UP);
        } else {
            if (dy > 0) priorityDirs.add(Direction.DOWN);
            else priorityDirs.add(Direction.UP);
            if (dx > 0) priorityDirs.add(Direction.RIGHT);
            else if (dx < 0) priorityDirs.add(Direction.LEFT);
        }
    ...
    protected boolean isSafeDirection(Snake snake, Direction dir, int boardWidth, int boardHeight) {
        Point nextPos = snake.getNextPosition(dir);
...
class HardAI extends AIPlayer {
    ...
    private Direction findPathToApple(Snake snake, Point apple, int boardWidth, int boardHeight) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (node.depth < 5) {
                for (Direction dir : Direction.values()) {
                    Point nextPos = getNextPosition(node.position, dir, Snake.UNIT_SIZE);
    ...
    private Point getNextPosition(Point current, Direction dir, int unitSize) {
        ...
        switch(dir) {
            case UP: next.y -= unitSize; break;
            case DOWN: next.y += unitSize; break;
            case LEFT: next.x -= unitSize; break;
            case RIGHT: next.x += unitSize; break;
        }
        return next;
    }
